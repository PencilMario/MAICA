...
class FscUsersFuncMixin():
    ...
    async def _service_only_user(self) -> SqlUser:
        """Return the single trusted-local account, creating it once if needed."""
        stmt = sqlalchemy.select(SqlUser).where(SqlUser.username == "service_only")
        async with DatabaseUtils.SessionAuth() as session:
        ...
        writable_session = async_sessionmaker(
            DatabaseUtils.engine_auth,
            expire_on_commit=False,
        )
        try:
            async with writable_session() as session, session.begin():
                session.add(user)
    ...
    async def login(self, crid_b64: Optional[str] = None):
        ...
        """
        service_only = G.A.SERVICE_ONLY == "1"
        if service_only:
            obj = await self._service_only_user()
            user_id = obj.id
